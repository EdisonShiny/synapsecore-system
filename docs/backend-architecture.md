...
- AI engine: Z.AI GLM chat completions through `ZAI_API_KEY`, `ZAI_MODEL`, and `ZAI_BASE_URL`
- AI fallback rule: if `ZAI_API_KEY` is missing, AI stays in mock mode even when real mode is requested
...
2. The same service sends the unstructured input to Z.AI GLM and stores structured `ai_analysis`
...
3. `POST /ai/generate-phase-plan` asks Z.AI GLM to generate and store a structured `plan`
4. `POST /ai/validate-phase-plan` asks Z.AI GLM to store `validation` with missing information, unsupported claims, contradictions, and review level
...
2. Outcome review runs through Z.AI GLM inside the same module
...
1. `POST /approvals/request` asks Z.AI GLM for an approval recommendation and creates an `approval`
