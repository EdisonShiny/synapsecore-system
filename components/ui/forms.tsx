...
const fieldBase =
  "synapse-focus w-full rounded-xl border border-synapse-border bg-synapse-elevated px-3 py-2.5 text-body text-synapse-text placeholder:text-synapse-muted transition hover:border-slate-600 focus:border-synapse-secondary";
...
export function FileUploadBox({ label = "Upload files", hint }: { label?: string; hint?: string }) {
  return (
    <label className="synapse-focus flex min-h-36 cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-synapse-border bg-synapse-elevated px-6 py-8 text-center transition hover:border-synapse-secondary">
...
export function FilterBar({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-synapse-border bg-synapse-elevated p-3 md:flex-row md:items-center md:justify-between">
      {children}
    </div>
  );
}
